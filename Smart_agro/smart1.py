...
from wapi_controller import Controller
...
def auto():
	...
	return render_template('auto.html', title="AUTO", status=status)
	GPIO.cleanup()

@app.route('/wapi-index')
def wapi_index():
	return render_template('wapi_index.html')
...
def wapi():
	if request.method == 'POST': 
	   ...
	   geo_location = wapi.getLocation(input_location)
	   if geo_location == None:
		   wapi_address = "Unknown location"
		   report_template = render_template('reports.html', weather_address=wapi_address)
		   return report_template
	   
	   wapi_address = geo_location.address       
	   wapi_reports = wapi.getWeatherReports(data, geo_location)

	   report_template = render_template('reports.html', weather_address=wapi_address, weather_reports=wapi_reports)
